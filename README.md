...
사이클 01에서는 의도적으로 최근 24시간의 지진이라는 한 가지 현상만 다뤘습니다.
현재 공개 데모는 <https://quake.jisung.lol/>에서 확인할 수 있습니다. 사이클
02에서는 시간, 규모, 깊이
필터를 클라이언트에 추가했습니다. API에서 받은 원본 스냅샷은 유지하면서 하나의 필터
결과로 목록, 지도, 통계, 현재 선택을 파생하고, 선택한 필터 상태는 공유 가능한
URL에 보존합니다. 사이클 03에서는 FastAPI OpenAPI 문서와 TypeScript 생성물 사이의
불일치를 로컬과 CI에서 자동으로 차단합니다. 사이클 04에서는 Python 3.12를 기본
실행 버전으로 고정하고, `uv.lock`으로 모든 전이 의존성을 잠가 개발 환경·CI·API
컨테이너가 같은 `uv.lock`의 환경별 해석 결과를 사용하도록 만들었습니다. 사이클
05에서는 제품 화면과 상세 구현 로그 사이에 `/project` 소개 페이지를 추가해
...
- 품질: 결정론적 테스트 데이터, 계약 테스트, 브라우저 E2E, 로컬 통합 스택 검증, CI

사이클 02 로컬 측정에서 당시 UI가 받은 스냅샷은 약 45KB였으므로 필터는 브라우저에서
처리합니다. gzip으로 압축한 응답이 250KB를 넘거나 중급 모바일에서 필터 계산의
95백분위(p95)가 50ms를 넘으면 API 쿼리와 페이지네이션 승격을 다시 검토합니다. 필터 토글은
...
명령은 각 npm 워크스페이스로 위임하므로 기존 개발·검증 명령은 그대로 사용할 수
있습니다. Python 서비스와 생성 클라이언트도 각각 독립적으로 테스트할 수 있습니다.
...
- `http://localhost:3000/build-log`: 사이클별 구현·결함·검토 기록

운영 환경과 유사한 로컬 환경은 다음과 같이 실행합니다.
...
`test:e2e`는 운영용 앱을 빌드한 뒤 URL 필터 보존, 새로고침 복원, 초기화,
유효하지 않은 값의 정규화, Chromium 데스크톱·모바일 동작 일치를 확인합니다.
GitHub Actions는 코드를 푸시하거나 풀 리퀘스트를 열 때마다 API와 웹 작업을 분리해
실행합니다.
첫 원격 실행인
[Verify #30410815313](https://github.com/lulupang2/QuakeCurrent/actions/runs/30410815313)에서
커밋 `dce4594`의 두 작업이 모두 통과했습니다. 이는 CI 실행 증거이며 배포 또는
운영 환경 성능을 의미하지 않습니다.

## 프로젝트 소개 동선

채용용 진입점은 `/project`입니다. 소개 페이지는 클라이언트 상태와 지도 WebGL을
직접 포함하지 않는 서버 렌더링 컴포넌트(`Server Component`)이며, 한 화면 흐름에서 다음
증거로 연결합니다.

- `프로토타입 열기`: URL 필터와 3D·2D 지도를 직접 확인
- `구현·검증 로그 보기`: 각 사이클의 결함, 판단과 수정 근거 확인
- `GitHub에서 코드 보기`: 공개 소스 확인
- `GitHub Actions 확인`: 현재 소개 페이지를 포함한 공개 CI 실행 확인
...
커밋 `75d7be2`의 Python 3.12·3.13 계약 검사, 잠금 기반 API 컨테이너와
...
7.13.0`으로 실행 시점 의존성이 없는 타입을 생성합니다.
...
두 불일치 방지 검사와 기존 API·웹·브라우저 검증이 모두 통과했습니다.
